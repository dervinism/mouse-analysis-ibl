...
ephysroot = 'C:\Users\44079\data_repositories\ibl-eightprobes';
% mouse names
...
for imouse = 1:3
  ...
  ispont = tstart(imouse)*srate : min(floor(tVid(end)*srate), size(stall,2)-4);
  stall = stall(:,ispont);
  tspont = ispont / srate; % put back in seconds
